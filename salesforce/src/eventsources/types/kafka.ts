...
class DataSource extends GSDataSource {
  ...
  protected async initClient(): Promise<PlainObject> {
    const sslConfig: PlainObject = {
      rejectUnauthorized: false || this.config?.ssl?.reject, // Optional, based on your requirements
    };

    // Conditionally add SSL key, certificate, and CA if they are defined
    if (this.config.ssl.reject) {
      sslConfig.rejectUnauthorized = this.config.ssl.reject;
    }
