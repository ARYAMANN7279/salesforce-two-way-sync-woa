...
function _ref() {
    _ref = _async_to_generator(function*(pathString) {
        ...
        if (datasourcesConfigs && !Object.keys(datasourcesConfigs).length) {
            _logger.logger.fatal(`There are no datasources defined in datasource dir: ${pathString}`);
            process.exit(1);
        }
        const datasources = {};
        {
            ...
            try {
                for(var _iterator = _async_iterator(Object.keys(datasourcesConfigs)), _step; _iteratorAbruptCompletion = !(_step = yield _iterator.next()).done; _iteratorAbruptCompletion = false){
                    ...
                    const dsName = _value;
                    _logger.logger.info('Evaluating the inline yaml scripts in datasource config %s', dsName);
                    const location = {
                        datasource_name: dsName
                    };
                    datasourcesConfigs[dsName] = (0, _expandVariables.default)(datasourcesConfigs[dsName], location);
                    _logger.logger.info('Evaluated the inline yaml scripts for datasource config %s', dsName);
                    _logger.logger.debug('Evaluated script %o', datasourcesConfigs[dsName]);
                    ...
                    if (!fileName) {
                        _logger.logger.warn(`Did not find any datasource 'type' key defined in ${dsName}.yaml. Ignoring this file.`);
                        continue;
                    }
                    yield Promise.resolve(_path.default.join(pathString, 'types', `${fileName}`)).then((p)=>/*#__PURE__*/ _interop_require_wildcard(require(p))).then(function() {
                        var _ref = _async_to_generator(function*(Module) {
                            ...
                            if (!Constructor) {
                                _logger.logger.fatal('Expecting datasource %s module file to export GSDataSource under the DataSource or default key', dsName);
                            }
                            try {
