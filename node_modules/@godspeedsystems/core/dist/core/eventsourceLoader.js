...
function _ref() {
    _ref = _async_to_generator(function*(eventsourcesFolderPath, datasources) {
        ...
        if (eventsourcesConfigs && !Object.keys(eventsourcesConfigs).length) {
            _logger.logger.fatal(`There are no event sources defined in eventsource dir: ${eventsourcesFolderPath}`);
            process.exit(1);
        }
        const eventSources = {};
        {
            ...
            try {
                for(var _iterator = _async_iterator(Object.keys(eventsourcesConfigs)), _step; _iteratorAbruptCompletion = !(_step = yield _iterator.next()).done; _iteratorAbruptCompletion = false){
                    ...
                    _logger.logger.debug('evaluating event source %s', esName);
                    const location = {
                        eventsource_name: esName
                    };
                    eventsourcesConfigs[esName] = (0, _expandVariables.default)(eventsourcesConfigs[esName], location);
                    _logger.logger.debug('evaluated eventsource %s %o', esName, eventsourcesConfigs[esName]);
                    const fileName = eventsourcesConfigs[esName].type;
                    try {
                        const Module = yield Promise.resolve(_path.default.join(eventsourcesFolderPath, 'types', fileName)).then((p)=>/*#__PURE__*/ _interop_require_wildcard(require(p)));
                        ...
                        if (isPureEventSource) {
                            eventSourceInstance = new Constructor(eventsourcesConfigs[esName], datasources);
                            if ('init' in eventSourceInstance) {
                                yield eventSourceInstance.init();
                            }
                        } else {
                            ...
                            if (!correspondingDatasource) {
                                _logger.logger.fatal(`Corresponding data source for event source ${esName} is not defined. Please ensure a data source type exists with the same file name in src/datasources directory`);
                                process.exit(1);
                            } else {
                                eventSourceInstance = new Constructor(eventsourcesConfigs[esName], correspondingDatasource.client);
                            }
                        }
                        eventSources[esName] = eventSourceInstance;
                    } catch (error) {
                        _logger.logger.error('Failed to load event source %s %s %o', esName, error.message, error);
                        process.exit(1);
                    }
                }
            } catch (err) {
